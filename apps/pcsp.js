...
function initPCSPDynamicUI(){
  initPCSPDayBtns();initPCSPAdlList();renderPlanningPeople();renderPCSPContacts();renderPCSPRisks();renderPCSPGoals();renderSadcActivityChips();renderSelectedSadcActivities();renderPCSPCommunity();renderPCSPRights();pcspMarkConfirmFields();
}
...
function getMedLibrary(){var custom=[];try{custom=JSON.parse(localStorage.getItem('med_custom')||'[]');}catch(e){}return MED_LIBRARY.concat(custom);}
function saveMedToLibrary(name,reason){var custom=[];try{custom=JSON.parse(localStorage.getItem('med_custom')||'[]');}catch(e){}if(!custom.some(function(m){return m.name.toLowerCase()===name.toLowerCase();})&&!MED_LIBRARY.some(function(m){return m.name.toLowerCase()===name.toLowerCase();})){custom.push({name:name,reason:reason||''});localStorage.setItem('med_custom',JSON.stringify(custom));}}
...
function pcspFactsForAI(){var e=collectPCSPEntry();return {
  ...
  capacity:e.functional,selectedSadcActivities:e.sadcActivities,planning:e.planning,workInterest:e.workVolunteer.interest,
  existingPersonCentered:e.personCentered
};}
function pcspAiBaseInstruction(){return 'Use ONLY the facts supplied. Do not invent family members, religion, culture, preferences, employment interests, housing preferences, planning attendees, risks, rights restrictions, diagnoses, or communication preferences. When a participant-specific fact is missing, return exactly "CONFIRM WITH PARTICIPANT" for that field. Write final PCSP narrative in professional plain English, person-centered and concise.';}
async function aiDraftWholePCSP(){
  ...
  try{var facts=pcspFactsForAI();var prompt=`You are drafting a NYS SADC Person-Centered Service Plan. ${pcspAiBaseInstruction()}
...
Create 2-3 reasonable draft goals only from documented health/support facts and selected activities. Goals are proposals and require participant confirmation. Never invent a natural support person; use CONFIRM WITH PARTICIPANT when unknown.`;
    var obj=await _callAIForJSON(prompt);if(obj.personCentered){Object.keys(obj.personCentered).forEach(function(k){if(document.getElementById('p-pc-'+k))pcspSet('p-pc-'+k,obj.personCentered[k]);});}
...
async function aiDraftPersonCentered(){
  ...
  try{var facts=pcspFactsForAI();var prompt=`You are drafting the person-centered narrative section of a NYS SADC PCSP. ${pcspAiBaseInstruction()}\nFACTS:\n${JSON.stringify(facts)}\nReturn ONLY valid JSON with keys: importantTo, importantFor, strengthsAbilities, interests, personalPreferences, servicePreferences, staffPreferences, settingPreferences, medicalNeeds, behavioralNeeds, socialNeeds, communityNeeds, transportationNeeds, housingPreferences, culturalNeeds, linguisticNeeds, communicationNeeds.`;var obj=await _callAIForJSON(prompt);Object.keys(obj||{}).forEach(function(k){if(document.getElementById('p-pc-'+k))pcspSet('p-pc-'+k,obj[k]);});pcspMarkConfirmFields();}
  catch(e){alert('❌ AI 작성 실패: '+e.message);}finally{if(btn){btn.disabled=false;btn.textContent='✨ 이 섹션 전체 작성';}}
}
async function aiDraftGoals(){
  try{var facts=pcspFactsForAI();var hint=prompt('참여자가 말한 목표 방향/키워드가 있으면 입력하세요.\n없으면 비워도 됩니다.','');if(hint===null)return;var prompt=`Draft 2-3 SMART goals for a NYS SADC PCSP. ${pcspAiBaseInstruction()}\nFACTS:\n${JSON.stringify(facts)}\nParticipant/staff goal hints: ${hint||'none provided'}\nReturn ONLY valid JSON: {"goals":[{"goal":"","outcome":"","targetDate":"YYYY-MM-DD or CONFIRM WITH PARTICIPANT","frequency":"","actions":"","activities":"","responsiblePerson":"","naturalSupport":"","paidSupport":"","staffResponsibility":"","progressReviewMethod":""}]}. Do not claim that the participant chose a goal unless the hint/facts support it.`;var obj=await _callAIForJSON(prompt);if(Array.isArray(obj.goals)){obj.goals.forEach(function(g){var x=Object.assign(emptyGoal(),g||{});x.needsConfirmation=true;_pcspGoals.push(x);});renderPCSPGoals();}}
  catch(e){alert('❌ AI 목표 생성 실패: '+e.message);}
}
async function aiDraftGoalItem(i){var g=_pcspGoals[i];if(!g)return;try{var hint=prompt('이 Goal에 반영할 참여자/직원 확인 키워드','');if(hint===null)return;var prompt=`Complete one SMART goal in a NYS SADC PCSP. ${pcspAiBaseInstruction()}\nFACTS:\n${JSON.stringify(pcspFactsForAI())}\nCurrent goal:\n${JSON.stringify(g)}\nHint: ${hint||'none'}\nReturn ONLY one JSON object with keys goal,outcome,targetDate,frequency,actions,activities,responsiblePerson,naturalSupport,paidSupport,staffResponsibility,progressReviewMethod.`;var obj=await _callAIForJSON(prompt);_pcspGoals[i]=Object.assign(emptyGoal(),g,obj||{});_pcspGoals[i].needsConfirmation=true;renderPCSPGoals();}catch(e){alert('❌ AI Goal 작성 실패: '+e.message);}}
async function aiDraftTextField(targetId,label){var el=document.getElementById(targetId);if(!el)return;var hint=prompt(label+'에 반영할 사실/키워드를 입력하세요.\n현재 입력된 내용이 있으면 Claude가 함께 사용합니다.','');if(hint===null)return;try{var prompt=`Write the "${label}" field for a NYS SADC PCSP. ${pcspAiBaseInstruction()}\nFACTS:\n${JSON.stringify(pcspFactsForAI())}\nExisting field text: ${el.value||''}\nAdditional confirmed hint: ${hint||'none'}\nReturn ONLY the field text, no heading, no markdown.`;var res=await apiCall({action:'aiPCSP',prompt:prompt});if(!res||!res.ok||!res.data||!res.data.success)throw new Error((res&&res.data&&res.data.error)||'AI response error');el.value=pcspCleanAIText(res.data.text);pcspAutoHeight(el);pcspMarkConfirmFields();}catch(e){alert('❌ AI 작성 실패: '+e.message);}}
async function aiFillRiskItem(i){var r=_pcspRisks[i];if(!r)return;if(!String(r.risk||'').trim()){alert('먼저 실제로 확인된 Risk를 입력해주세요.');return;}try{var prompt=`Draft supporting risk-management wording for a NYS SADC PCSP. ${pcspAiBaseInstruction()}\nFACTS:\n${JSON.stringify(pcspFactsForAI())}\nConfirmed risk: ${r.risk}\nReturn ONLY JSON {"trigger":"","response":"","measure":"","safeguard":""}. Do not invent past events or triggers; use CONFIRM WITH PARTICIPANT when unknown.`;var obj=await _callAIForJSON(prompt);Object.assign(r,obj||{});renderPCSPRisks();pcspMarkConfirmFields();}catch(e){alert('❌ AI 작성 실패: '+e.message);}}
async function aiFillCommunityItem(i){var c=_pcspCommunity[i];if(!c||!c.activity){alert('먼저 실제 관심 활동을 입력해주세요.');return;}try{var prompt=`Draft wording for a confirmed community activity in a NYS SADC PCSP. ${pcspAiBaseInstruction()}\nFACTS:\n${JSON.stringify(pcspFactsForAI())}\nConfirmed community activity: ${c.activity}\nExisting: ${JSON.stringify(c)}\nReturn ONLY JSON {"details":"","location":"","schedule":"","materials":"","transportation":"","supports":""}. Do not invent location/schedule/transportation; use CONFIRM WITH PARTICIPANT when unknown.`;var obj=await _callAIForJSON(prompt);Object.assign(c,obj||{});renderPCSPCommunity();pcspMarkConfirmFields();}catch(e){alert('❌ AI 작성 실패: '+e.message);}}
async function aiDraftWorkVolunteer(){var interest=pcspVal('p-work-interest');if(!interest){alert('먼저 Work/Volunteer 관심 여부를 선택해주세요.');return;}try{var hint=prompt('참여자가 말한 관심 분야/일정 등 확인된 사실이 있으면 입력하세요.','');if(hint===null)return;var prompt=`Draft Work/Volunteer fields for a NYS SADC PCSP. ${pcspAiBaseInstruction()}\nFACTS:\n${JSON.stringify(pcspFactsForAI())}\nConfirmed interest selection: ${interest}\nConfirmed hint: ${hint||'none'}\nReturn ONLY JSON {"opportunity":"","frequencySchedule":"","supportNeededProvided":"","transportation":"","unableReason":""}. If interest is no, do not invent an opportunity. If na, focus on unableReason only.`;var o=await _callAIForJSON(prompt);pcspSet('p-work-opportunity',o.opportunity||'');pcspSet('p-work-frequency',o.frequencySchedule||'');pcspSet('p-work-support',o.supportNeededProvided||'');pcspSet('p-work-transport',o.transportation||'');pcspSet('p-work-unable',o.unableReason||'');pcspMarkConfirmFields();}catch(e){alert('❌ AI 작성 실패: '+e.message);}}
async function aiPolishRight(i){var r=_pcspRights[i];if(!r||r.modified!=='Yes'){alert('Modification Needed = Yes인 권리에서만 사용할 수 있습니다.');return;}var missing=['description','diagnosisCondition','priorInterventions','dataReviewMethod','reviewTimeframe','noHarmAssurance'].filter(function(k){return !String(r[k]||'').trim();});if(missing.length){alert('AI가 사실을 만들지 않도록 먼저 6개 근거 항목을 간단히 입력해주세요.\n비어 있는 항목: '+missing.join(', '));return;}try{var prompt=`Polish the following confirmed HCBS rights modification facts into concise professional English without changing meaning or inventing anything. Return ONLY JSON with the same keys.\nRight: ${r.right}\nFacts: ${JSON.stringify(r)}`;var o=await _callAIForJSON(prompt);['description','diagnosisCondition','priorInterventions','dataReviewMethod','reviewTimeframe','noHarmAssurance'].forEach(function(k){if(o[k])r[k]=o[k];});renderPCSPRights();}catch(e){alert('❌ AI 문장 정리 실패: '+e.message);}}
