...
function editPCSP(id){
  ...
  loadJSONfromDrive(memberId,memberName,'PCSP').then(function(res){
    if(res&&res.ok&&res.data&&res.data.found&&res.data.data){
      ...
      alert('저장된 PCSP 상세 JSON을 Drive에서 찾을 수 없습니다.');
    }
  }).catch(function(e){alert('PCSP 불러오기 실패: '+e.message);});
}
...
async function loadMedLibraryFromSheets(){try{var res=await apiGet({action:'read',sheet:'medlib'});PCSP_MED_CUSTOM=(res&&res.ok&&res.data?res.data:[]).map(function(r){return {name:String(r['이름']||''),reason:String(r['이유']||'')};}).filter(function(m){return m.name;});_pcspMedLibLoaded=true;}catch(e){console.log('약물 라이브러리 로드 실패:',e);}}
function getMedLibrary(){if(!_pcspMedLibLoaded)loadMedLibraryFromSheets();return MED_LIBRARY.concat(PCSP_MED_CUSTOM);}
async function saveMedToLibrary(name,reason){name=String(name||'').trim();if(!name)return;var exists=getMedLibrary().find(function(m){return m.name.toLowerCase()===name.toLowerCase();});if(exists)return;try{var id='MED_'+name.toLowerCase().replace(/[^a-z0-9]+/g,'_').replace(/^_|_$/g,'');await apiCall({action:'upsert',sheet:'medlib',key:'ID',value:id,data:{'ID':id,'이름':name,'이유':reason||''}});PCSP_MED_CUSTOM.push({name:name,reason:reason||''});}catch(e){console.log('약물 라이브러리 저장 실패:',e);}}
